...
import ReactImageMagnify from "react-image-magnify";
import { useCart } from "../context/context";
import { getBrandDetailsById } from "../api/Api";
...
const ProductDetails = () => {
  ...
  console.log("entry to test");

  useEffect(() => {
    getProductDetailsById(route);
  }, [route]);

  const getProductDetailsById = async (data) => {
    const response = await getBrandDetailsById(data?.id);
    ...
    <div className="w-11/12 xl:w-4/5 m-auto py-10 space-y-6">
      <div className="flex   md:flex-row items-center space-x-0 md:space-x-10">
        ...
        <div className="space-y-4 mt-4 md:mt-0 w-[60%]">
          ...
          <p className="text-gray-500">
            <strong> Description: {product.branddescription}
            </strong>
          </p>
